...
class RDFMapper {
  ...
  * triples (linkDump) {
    const metaFields = linkDump.metaFields
  ...
  * metaFieldTriples (metaFields) {
    ...
    if (mf.PREFIX.uriSpace) {
      yield triple(sourceset, voi('uriSpace'), mf.PREFIX.uriSpace)
    } else {
        // TODO
        // yield triple(sourceset, voi('uriRegexPattern'), mf.PREFIX.uriRegexPattern)
    }

    if (mf.TARGET.uriSpace) {
      yield triple(targetset, voi('uriSpace'), mf.TARGET.uriSpace)
    } else {
        // TODO
        // yield triple(targetset, voi('uriRegexPattern'), mf.TARGET.uriRegexPattern)
    }

    if (typeof mf.RELATION === 'string') {
      yield triple(dump, voi('linkPredicate'), mf.RELATION)
    }
...
// factory method for class RDFMapper
